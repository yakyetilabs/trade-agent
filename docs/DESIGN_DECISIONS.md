...
Investigation proved that assumption false: the reference is a fully public demo whose only spend protection is infra-level (reserved concurrency, a billing alarm, a cheap model, per-request caps, and pre-model guards).
...
(The reference project could use edge rate limiting only because it has no SSE.)
