# Product Overview: Who Uses trade-agent, and What Happens When They Do
...
trade-agent puts an LLM inside a regulated-enterprise compliance workflow without handing it the keys.
...
The backend (FastAPI on Cloud Run, the agent loop, the four tools, the guards, the trace store, the eval suite) is implemented.
The analyst-facing React interface that frames this interaction (including the `/traces` audit view) is planned and not yet built.
