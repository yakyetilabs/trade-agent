...
- **Periodic review: Eval suite against the deployed agent.** An evaluation dataset sitting at `backend/eval/cases.json` exercises the system across 5 clear categories (tariff classification, import flags, compliance queries, escalation triggers, and scope violations) with 30 distinct test cases. Each run evaluates our production model (Gemini 2.0 Flash) against our benchmark comparison model (Gemini 1.5 Pro), outputting cost/accuracy matrices to justify our production layout choices.
